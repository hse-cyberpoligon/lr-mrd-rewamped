...
Получаем скомпилированный (исполняемый) файл `main.bin`.

Переносим исполняемый файл на рабочий стол пользователя. Меняем владельца, группу на `root`. Добавляем право исполнения от админа `chmod +s` на исполняемый файл.

Создаем лаунчер в виде sh скрипта:
```
#!/bin/sh
/home/student/Desktop/Checker.bin
```
Добавляем лаунчер в автозапуск (через одноименную утилиту Астры).
